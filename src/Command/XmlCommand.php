...
class XmlCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $classMetadataFactory = new ClassMetadataFactory(new XmlFileLoader('fixtures/ihk.xml'));
        ...
        if ($filePath) {
            ...
            $filePath = 'fixtures/ihk.xml';
        }

        if (file_exists('fixtures/ihk.xml')) {

           $counter = $this->importIHKXml();
    ...
    private function importIHKXml(){
        $file = file_get_contents('fixtures/ihk.xml');
                ...
                if ($elm2->getName() === 'questions'){
                    ...
                    foreach ($elm2->children() as $elm3){
                        ...
                        foreach ($elm3->children() as $elm4){
                            // if property is choices
                            if ($elm4->getName() === 'choices'){
                                ...
                                foreach ($elm4->children() as $elm5) {
                                    ...
                                    foreach ($elm5->children() as $elm6) {
                                        ...
                                        $choice->{$pName}( strip_tags(trim($elm6)));
                            ...
                            }else if($elm4->getName() === 'correctAnswer'){ //add correctAnswer property to question
                                $correctChoice = new Choice();

                                foreach ($elm4->children() as $elm6) {
                                    $pName = 'set' . ucfirst($elm6->getName());
                                    $correctChoice->{$pName}( strip_tags(trim($elm6)));
                                    //$answer->{$pName}(  str_replace(array("\n", "\r"), '', strip_tags(trim($elm6))));
                                }
